...
module.exports = {
	save: function(req, res){
		...
		var clicks = req.param('clicks');
		var username = req.session.username;
		
		Pr_amount.create({username: username, amount: amount, clicks: clicks}).exec(function (err, amount) {
	  	      if (err) {
		  	        return res.json(200, {error_message: 'Invalid parameter: ' + err});
		  	  }
	  	      if (amount) {
	  	        res.json(200, {user: amount, success_message:'Your Data has been save to the db'});
	  	      }
		});
	},
