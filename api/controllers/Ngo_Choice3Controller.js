...
module.exports = {
	save: function (req, res){
		...
		Ngo_Choice3.findOne({username: username }).exec(function (err, record) {
			if(err){
				//console.log(err);
			...
			if(record){
				...
				Ngo_Choice3.update({id:record.id},{ ngo: ngo_choice1 }).exec(function updateCB(err, updated) {
					if(err){
						console.log(err);
						res.json(200, {message_error: err});
					}
				...
				Ngo_Choice3.create({username: username, ngo: ngo_choice1 }).exec(function (err, ngo) {
					if(err){
						//console.log(err);
						res.json(200, {message_error: err});
					}
	...
	find:  function (req, res){
		var username = req.session.username;
		Ngo_Choice3.findOne({username: username }).exec(function (err, choice1) {
			if(err){
				//console.log(err);
				res.json(200, {message_error: err});
			}
